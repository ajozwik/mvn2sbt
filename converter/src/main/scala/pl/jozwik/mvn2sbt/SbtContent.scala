...
object SbtContent {
  ...
  private def toProjectContent(project: Project, path: String, settings: Set[String], projectName: String, dependencies: String, dependsOnString: String) = if (path.isEmpty) {
    s"""
  ...
  private def createAliasForDependency(accCache: Map[GroupArtifact, (MavenDependency, String)], md: MavenDependency, groupArtifact: GroupArtifact) = accCache.get(groupArtifact) match {
