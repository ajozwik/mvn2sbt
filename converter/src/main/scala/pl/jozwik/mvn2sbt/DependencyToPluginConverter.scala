...
object DependencyToPluginConverter extends LazyLogging {
  ...
  def addSeqToArray(equal: Boolean): String =
    if (equal) {
      ""
    }
    else {
  ...
  def addPluginForDependency(rootPath: File, path: File, dependencies: Set[Dependency]): (Set[String], Set[String]) =
    dependencies.foldLeft((Set.empty[String], Set.empty[String])) { (tuple, dependency) =>
      dependency.mavenDependency match {
        case MavenDependency("org.testng", _, _) =>
          ...
          (
            tuple._1 ++ Set(s"de.johoop.testngplugin.TestNGPlugin.testNGSettings$endSettings", s"""testNGSuites := Seq("$rp")"""),
