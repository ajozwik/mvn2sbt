...
object DirProjectExtractor extends LazyLogging {
  ...
  private[mvn2sbt] def createProjectMap(dir: File, pomModel: Model, parent: Option[MavenDependency]): Map[MavenDependency, ProjectInformation] = {
    ...
    val resolvers        = resolversOptions.fold(Seq.empty[String]) { seq =>
      seq
    }
  ...
  private def toPomParent(parent: Option[Parent]) =
    parent.map(p => MavenDependency(extractOption(p.groupId), extractOption(p.artifactId), extractOption(p.version)))
  ...
  private def toProjectMap(dir: File, parent: Option[MavenDependency]) = {
    ...
    pomOption
      ...
      .getOrElse(sys.error(s"""$EFFECTIVE_POM_XML file missing in ${dir.getAbsolutePath}, run "scala Eff.sc ${dir.getAbsolutePath}" first"""))
  }
